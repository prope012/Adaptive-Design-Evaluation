...
#   "tlogit" - logit transformation with independent t prior distributions on coefficients
...
#tlr and tlogit mcmc sampler specification
...
simulate.trial <- function(response.probs, ns, max.ar=0.75, rand.type=c("Coin","Urn","Block"), max.deviation=3, model=c("tlr","ibb","tlogit"),
...
# however, we also evaluated the below statistics using the 5 other probability models and 2 other randomization methods
#   The other models: ibb with prior mean 0.12, logistic regression with prior intercept locations of 
#   0 and log(0.12/0.88), logit transformation with prior locations of 0 and log(0.12/0.88)
#   The other randomization methods: MPBD and MWUD
# When implementing the logistic regression or logit transformation models, we recommend using a parallel
# computation package to increase compuation speed (unparallelized code below)
...
simulation.analysis = function(null.response.probs, ns, max.ar=0.75, rand.type=c("Coin","Urn","Block"), max.deviation=3, model=c("tlr","ibb","tlogit"),
  ...
  # proportion of nE - nC < 0, i.e, proportion of trials with more individuals assigned to the inferior
