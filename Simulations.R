...
# Function to simulate Bayesian Group Sequential Response Adaptive Randomization trial
...
# ntrials = the desired number of simulations
...
simulation.analysis = function(null.response.probs, ns, max.ar=0.75, rand.type=c("Coin","Urn","Block"), max.deviation=3, model=c("tlr","ibb"),
  ...
  # run simulations at the desired null response rates
  Results = list()
  ...
  # compute type I error using inputted posterior probability stopping boundary
  ...
  # compute power using inputted posterior probability stopping boundary and hypothesized alternative
  ...
  alt.foo.15 = sapply(Alt.Scenario,function(x){
    stop.at = which.max(x[,1] > c(bounds[-length(ns)],0)| x[,1] < (1-bounds)) #determine when trial exceeds posterior probability stopping boundary for the first time
