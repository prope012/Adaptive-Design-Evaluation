...
# Control Arm, i.e., Prob(pi_{Treatment} > pi_{Control} | Data), using either the independent 
# Beta-Binominal (ibb) model or the Logistic Regression Model
...
# The first function below implements the logistic regression model with independent 
# t(degrees of freedom, location, scale) priors on each model parameter via a Polya-Gamma 
# Gibbs sampler
# The second function computes the Posterior Probability that the Treatment Arm is better than the 
# Control Arm using the tlr.sampler function
...
library(BayesLogit)
tlr.sampler = function(n=NULL,y=c(1,0),X=rbind(c(1,0.5),c(1,-0.5)),df=rep(7,ncol(X)),m0=rep(0,ncol(X)),s0=rep(2.5,ncol(X)),nsamps=2000,warmup=100){
...
tlr.post.prob <- function(n,y,df=c(7,7),m0=c(0,0),s0=c(2.5,2.5),nsamps=2000,warmup=100){
  post.prob = rowMeans(tlr.sampler(n=n,y=y,X=rbind(c(1,0.5),c(1,-0.5)),df=df,m0=m0,s0=s0,nsamps=nsamps,warmup=warmup)>0)[2]
  return(post.prob)
  }
